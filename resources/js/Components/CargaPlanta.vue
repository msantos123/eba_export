...
const carga = ref(Object.values(props.cargas).map(carga => ({
    ...
    kilosnetos: carga.kilosnetos,
    librasnetas: carga.librasnetas,
    costo_caja: carga.costo_caja,
})));

const totalCantidad = computed(() => carga.value.reduce((acc, current) => acc + current.cantidad, 0))
const totalKilosNetos = computed(() => carga.value.reduce((acc, current) => acc + parseFloat(current.kilosnetos || 0), 0))
const totalLibrasNetas = computed(() => carga.value.reduce((acc, current) => acc + parseFloat(current.librasnetas  || 0), 0))
...
const openModal = async (id, op) => {
    ...
    operation.value = op;
        try {
            const response = await axios.get(`/sigaSalidaDetalle/verCargaPlanta/${id}`);
...
<template>
    <InputLabel value="Datos de Solicitud de Carga a la Planta Industrial" class="text-lg font-maximo pb-2 border-b border-gray-300"></InputLabel>
    <br>
        ...
        <tbody>
            <tr v-for="car, i in cargas" :key="car.id" class="border-b">
                ...
                <td class="border-b border-gray-200 bg-white px-3 py-3 text-sm">
                    <BlueButton @click="$event=>openModal(car.id)">
