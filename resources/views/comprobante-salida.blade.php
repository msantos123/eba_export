...
<body>
    ...
    <table class="table1">
        <tr>
          <td>Codigo N°: </td>
          <td>{{ $comprobanteSalida->codigo_salida }}</td>
          <td></td>
          <td></td>
          <td>Fecha: </td>
          <td>{{ $comprobanteSalida->fecha_salida }}</td>
        </tr>
    </table>
    <table class="table2">
        <tr>
            <td colspan="4">DATOS DE SALIDA ALMACEN</td>
        <tr>
          <td>Nro. Factura: </td>
          <td>{{ $comprobanteSalida->factura }}</td>
          <td>Destino: </td>
          <td>{{ $comprobanteSalida->destino }}</td>
        </tr>
        <tr>
          <td>Contenedor: </td>
          <td>{{ $comprobanteSalida->contenedor }}</td>
          <td>Reserva: </td>
          <td>{{ $comprobanteSalida->reserva }}</td>
        </tr>
      </table>
    <table class="table2">
        <tr>
        ...
        <tr>
          ...
          <td>Camión: </td>
          <td>{{ $comprobanteSalida->camion }}</td>
        </tr>
        <tr>
          <td>Recibi de: </td>
          <td>{{ $comprobanteSalida->camion }}</td>
          <td>Placa: </td>
          <td>{{ $comprobanteSalida->placa }}</td>
        </tr>
        <tr>
          <td>Transporte: </td>
          <td>{{ $comprobanteSalida->empresa }}</td>
          <td>Licencia: </td>
          <td>{{ $comprobanteSalida->licencia }}</td>
        </tr>
        <tr>
          <td>Destino: </td>
          <td>{{ $comprobanteSalida->destino }}</td>
          <td>Celular: </td>
          <td>{{ $comprobanteSalida->celular }}</td>
        </tr>
      </table>
  <table class="table3">
    <tr>
    ...
    <tr class="total">
        ...
        <td>{{ $comprobanteSalida->total_cantidad }}</td>
        <td>{{ $comprobanteSalida->total_kilosnetos }}</td>
        <td>{{ $comprobanteSalida->total_librasnetas }}</td>
    </tr>
  </table>
  <table class="table5">
    <tr>
        <td>Observaciones: El producto de EBA arriba declarado son responsabilidad del transportista en el cuidado y entrega
            del producto (y contenedor si corresponde detallado arriba) hasta puerto de Arica - Chile. EBA no es responsable de\
            cualquier otra carga o el transporte, estos son propiedad de la empresa de transporte. Es prohibido el daño o alteracion
            de precinto si corresponde.
        </td>
    </tr>
  </table>
  <table class="table5">
    <tr>
        <td>Importante: Es responsabilidad de la empresa de transporte cualquier otra carga dentro del mencionado transporte que no
            sea propiedad de EBA. Todo producto EBA es entregado completamente cerrado/sellado cualquier alteración al envase y/o sello de
            contenedor, es responsabilidad de la empresa de transporte.
        </td>
    </tr>
  </table>
</main>
